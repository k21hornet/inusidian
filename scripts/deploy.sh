...
BATCH_IMAGE_NAME="inusidian-reminder-batch"
...
DOCKER_REGISTRY_BATCH_IMAGE_TAG="batch-${BATCH_IMAGE_TAG}"
...
if docker image inspect "${BATCH_IMAGE_NAME}:${BATCH_IMAGE_TAG}" > /dev/null 2>&1; then
    docker image rm "${BATCH_IMAGE_NAME}:${BATCH_IMAGE_TAG}"
fi
...
docker tag "${BATCH_IMAGE_NAME}:${BATCH_IMAGE_TAG}" "${DOCKER_REGISTRY_REPOSITORY_NAME}:${DOCKER_REGISTRY_BATCH_IMAGE_TAG}"
...
docker push "${DOCKER_REGISTRY_REPOSITORY_NAME}:${DOCKER_REGISTRY_BATCH_IMAGE_TAG}"
